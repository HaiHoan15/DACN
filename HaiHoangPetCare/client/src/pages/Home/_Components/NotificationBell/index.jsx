...
const NotificationBox = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const res = await api.get("get_user_notifications.php", {
  ...
  const handleNotificationClick = (e, type) => {
    e.preventDefault();
    e.stopPropagation();
    setOpen(false);
    
    // Lấy thông tin user và tạo slug
    const storedUser = localStorage.getItem("user");
    if (!storedUser) return;
    ...
    // Chuyển hướng trực tiếp đến trang tương ứng
    if (type === 'appointment') {
      window.location.href = `/nguoi-dung/${slug}?tab=schedule`;
    } else if (type === 'order') {
      window.location.href = `/nguoi-dung/${slug}?tab=order`;
    }
  };
          ...
          <div className="max-w-xs w-80 bg-white border border-gray-200 rounded-xl overflow-hidden shadow-[0_10px_25px_-5px_rgba(0,0,0,0.05),0_8px_10px_-6px_rgba(0,0,0,0.04)] hover:shadow-[0_20px_25px_-5px_rgba(0,0,0,0.08),0_15px_15px_-6px_rgba(0,0,0,0.06)] transition-all duration-300">
            ...
            <div className="px-4 py-3 bg-gray-50 border-b grid grid-cols-2 gap-2">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-xs text-gray-500 mt-1">Lịch hôm nay</p>
                <p className="font-bold text-blue-600">{notifications.todayAppointments}</p>
              </div>
              <div className="flex flex-col items-center text-center">
                <span className="text-2xl">📦</span>
                <p className="text-xs text-gray-500 mt-1">Đơn hàng</p>
                <p className="font-bold text-orange-600">{notifications.processingOrders}</p>
              </div>
            </div>
            ...
            <div className="divide-y divide-gray-100 max-h-96 overflow-y-auto">
              {notifications.items && notifications.items.length > 0 ? (
                notifications.items.map((n, index) => (
                  <div
                    key={index}
                    onClick={(e) => handleNotificationClick(e, n.type)}
                    className="p-3 hover:bg-yellow-50 transition-colors cursor-pointer"
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        {n.time && <p className="text-xs text-gray-400 mt-1">{formatTime(n.time)}</p>}
                      </div>
                    </div>
                  </div>
                ))
